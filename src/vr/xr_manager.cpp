...
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO,  "ETRXR", __VA_ARGS__)
#define LOGW(...) __android_log_print(ANDROID_LOG_WARN,  "ETRXR", __VA_ARGS__)
#define LOGE(...) __android_log_print(ANDROID_LOG_ERROR, "ETRXR", __VA_ARGS__)
#define LOGD(...) __android_log_print(ANDROID_LOG_DEBUG, "ETRXR", __VA_ARGS__)
...
/** The JNI handles the OpenXR loader needs. With SDL gone these come
 *  straight from the NativeActivity the glue owns; android_main() has
 *  already attached this thread to the VM. */
JNIEnv* androidJNIEnv()
{
    android_app* app = etr_platform::GetAndroidApp();
    if (!app || !app->activity)
        return NULL;
    JNIEnv* env = NULL;
...
{
    android_app* app = etr_platform::GetAndroidApp();
    return (app && app->activity) ? app->activity->clazz : NULL;
}
...
bool XRManager::initLoader()
{
    ...
    if (!env)
    {
        LOGE("No JNI environment.");
        return false;
    }
    if (env->GetJavaVM(&m_java_vm) != JNI_OK || !m_java_vm)
    {
        LOGE("Could not get JavaVM.");
        return false;
    }
    jobject activity = androidActivity();
    if (!activity)
    {
        LOGE("Could not get activity.");
        return false;
    }
    // activity->clazz is already a global reference owned by the glue, so
    // unlike SDL's local ref there is nothing to delete here.
    ...
    {
        LOGW("xrInitializeLoaderKHR not available.");
    }
    return true;
}   // initLoader
...
bool XRManager::createInstance()
{
    ...
    {
        LOGI("Runtime %s version %u.%u.%u", ip.runtimeName,
...
bool XRManager::createSession()
{
    ...
    LOGI("GLES %d.%d, runtime requires %u.%u - %u.%u", major, minor,
    ...
    if (gl_version < reqs.minApiVersionSupported)
        LOGW("GLES version below runtime minimum, trying anyway.");
    ...
    {
        LOGE("No current EGL display/context on this thread.");
        return false;
    }
    EGLint config_id = 0;
    if (!eglQueryContext(m_egl_display, m_egl_context, EGL_CONFIG_ID, &config_id))
    {
        LOGE("eglQueryContext(EGL_CONFIG_ID) failed: 0x%x",
                   eglGetError());
        return false;
    }
    ...
    if (!eglChooseConfig(m_egl_display, attribs, &config, 1, &num_configs) ||
        num_configs < 1)
    {
        LOGE("eglChooseConfig for config id %d failed: 0x%x",
    ...
    LOGI("Session created (EGL config id %d).", config_id);
    return true;
}   // createSession
...
int64_t XRManager::chooseSwapchainFormat()
{
    ...
    for (unsigned i = 0; i < count; i++)
        LOGD("Swapchain format 0x%llx", (long long)formats[i]);
                                ...
                                bool with_depth)
{
    ...
    for (unsigned i = 0; i < count; i++)
    {
        ...
        {
            LOGE("Swapchain FBO %u incomplete: 0x%x", i, status);
            glBindFramebuffer(GL_FRAMEBUFFER, prev_fbo);
            return false;
        }
    }
    glBindFramebuffer(GL_FRAMEBUFFER, prev_fbo);
    LOGI("Swapchain %ux%u format 0x%llx with %u images%s.", w, h,
...
bool XRManager::init()
{
    ...
    {
        LOGI("System '%s' vendor 0x%x, max swapchain %ux%u, "
    ...
    LOGI("GL_EXT_sRGB_write_control: %d", (int)m_has_srgb_write_control);
    ...
    if (view_count < 2)
    {
        LOGE("Stereo view configuration has %u views.", view_count);
        return false;
    }
    view_count = 2;
    for (int i = 0; i < 2; i++)
    ...
    for (int i = 0; i < 2; i++)
    {
        LOGI("View %d recommended %ux%u (max %ux%u), samples %u", i,
...
{
    LOGI("Session state %d -> %d", (int)m_session_state, (int)ev.state);
    ...
    case XR_SESSION_STATE_STOPPING:
    {
        ...
        if (XR_FAILED(r))
            LOGE("xrEndSession failed: %s", xrResultString(m_instance, r));
        m_session_running = false;
        m_views_valid = false;
...
bool XRManager::beginFrame()
{
    ...
    if (XR_FAILED(r))
    {
        LOGE("xrWaitFrame failed: %s", xrResultString(m_instance, r));
        return false;
    }
    ...
    if (XR_FAILED(r))
    {
        LOGE("xrBeginFrame failed: %s", xrResultString(m_instance, r));
        return false;
    }
    m_frame_begun = true;
...
// ----------------------------------------------------------------------------
bool XRManager::createScreenSwapchain(uint32_t width, uint32_t height)
{
    if (m_screen_swapchain.m_handle != XR_NULL_HANDLE &&
        m_screen_swapchain.m_width == width &&
        m_screen_swapchain.m_height == height)
    {
        return true;
    }
    destroySwapchain(m_screen_swapchain);
    // With depth, because the paused and reset screens draw the frozen 3D
    // scene through this same path.
    return createSwapchain(m_screen_swapchain, width, height, true);
...
void XRManager::endFrame()
{
    ...
    if (XR_FAILED(r))
    {
        ...
        if (error_count++ < 10)
            LOGE("xrEndFrame failed: %s", xrResultString(m_instance, r));
    }
    else
    {
        ...
        {
            first = false;
            LOGI("First frame submitted (%u layers).", count);
        }
    }
    m_frame_begun = false;
    m_submit_projection = false;
