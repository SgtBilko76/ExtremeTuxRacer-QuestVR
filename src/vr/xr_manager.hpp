//  Extreme Tux Racer -- OpenXR session manager
//
//  Adapted from the SuperTuxKart-VR prototype's XRManager, which is
//  engine-agnostic apart from its logging and shutdown hooks. Unlike the
//  original, the per-eye projection path here is actually driven: see
//  src/vr/xr_render.cpp.
...
public:
    ...
    /** Handles needed by the controller input layer, which builds its
     *  action set against this session. */
    XrInstance getInstance() const      { return m_instance; }
    XrSession  getSession() const       { return m_session; }
    XrSpace    getLocalSpace() const    { return m_local_space; }
    ...
    /** Allocates the quad-layer swapchain the 2D screens draw into. The
     *  original prototype only ever created this lazily inside
     *  presentFlatScreen(); driving the layer directly means it has to be
     *  created up front. */
    bool createScreenSwapchain(uint32_t width, uint32_t height);
